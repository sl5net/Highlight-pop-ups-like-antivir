...
; 
   ...
   ;ToolTip3sec(ImageSearchStrig . "`n`n" . A_LineNumber . " " . A_ScriptName . " " . Last_A_This)
   ;msgbox,% ImageSearchStrig . "`n`n" . A_LineNumber . " " . A_ScriptName . " " . Last_A_This
   ;~ ImageSearch, XPos, YPos, 0, 0, A_ScreenWidth, A_ScreenHeight,  %i%
   ...
   {
      ; 2 if there was a problem that prevented the command from conducting the search (such as failure to open the image file or a badly formatted option).
      tip := ImageSearchStrig . "`n`n" . " ErrorLevel = " . ErrorLevel . "`n  :( Die Suche konnte nicht durchgef¸hrt werden.  `n textInfo =`n" . textInfo
      ToolTip3sec(tip . "`n`n" . A_LineNumber . " " . A_ScriptName . " " . Last_A_This)
      msgbox, % tip . "`n`n" . A_LineNumber . " " . A_ScriptName . " " . Last_A_This
      Sleep,3000
      IfNotExist,%i%
         MsgBox,%i% existiert nicht. `n `n  %i% = i (line:%A_LineNumber%) `n 
      return false
